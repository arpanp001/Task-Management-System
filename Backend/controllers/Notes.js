...
const CreateNotes=async(req,res)=>{
    try {
        ...
        res.status(200).json({success:true,message:"Task created Successfully",Notes:CreateNotes})
    } catch (error) {
        console.log(error)
...
const UpdateNotes=async(req,res)=>{
    try {
      ...
      console.log("NotesUserId",NotesUserId)
        ...
        res.status(200).json({success:true,message:"Task Updates Successfully",UpdateNotes})
    ...
    try {
       ...
       return res.status(404).json({success:false,message:"Unauthorized user",})
        ...
        const Delete=await NotesModel.findByIdAndDelete(NotesId)
              ...
              res.status(200).json({success:true,message:"Task Deleted Successfully",Delete})
